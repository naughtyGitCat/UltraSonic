          ...
          <div className="col" style={{ gap: 10 }}>
            {progress.map(p => (
              <div key={p.agentId} className="row wrap" style={{ gap: 16, fontSize: 12 }}>
                ...
                {p.phase && <Badge>{p.phase === 'checking' ? 'Dedup check' : p.phase === 'hashing' ? 'Hashing' : p.phase === 'reclaiming' ? 'Reclaiming' : p.phase === 'moving' ? 'Moving' : p.phase === 'scanning' ? 'Scanning' : p.phase}</Badge>}
